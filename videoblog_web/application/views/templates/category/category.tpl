    ...
    <div class="span12">
        <div class="block">
            ...
            <div class="block-content collapse in">
				<form class="form-horizontal" id="frmcategory" action="{$data.base_url}category/{$data.function}" method="post" enctype="multipart/form-data">
					<input type="hidden" name="data[iCategoryId]" value="{$data.category.iCategoryId}">
					<fieldset>
						<legend>{if $data.function neq 'create'}Edit {else}Add {/if} Category</legend>
						...
						<div class="control-group" id="image">
							...
							<div class="controls">
								<input type="file" class="span10" id="vIcon" name="vIcon" value="{$data.category.vIcon}" onchange="CheckValidFile(this.value,this.name)">
							</div>
							<input type="hidden" id="editimage" value="{$data.category.vIcon}">
